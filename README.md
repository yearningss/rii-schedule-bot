...
* Нативные виджеты на рабочий стол (Android и iOS):
  * Android: нативный AppWidgetProvider с отображением текущей пары, статуса перемены, следующей пары, аудитории и поминутным AlarmManager.
  * iOS: нативное расширение WidgetKit (ScheduleWidget) с TimelineProvider и отображением расписания на главном экране.
  * Интерактивная кнопка ручного обновления и автоматическая фоновая синхронизация.
  * Совместимость со всеми популярными лаунчерами и оболочками (включая MagicOS, One UI, MIUI, Pixel Launcher, iOS 17+).
* Офлайн-кэширование:
...
* Оформление и темы:
  ...
  * Динамические сезонные и праздничные темы оформления и иконки приложения на рабочем столе (12 стилей: Новый год, Татьянин день, 23 февраля, 8 марта, День Победы, Выпускной, День города Рубцовска, День машиностроителя/АТЗ, весна, лето, осень, классический стиль РИИ).
  * Нативная смена иконки лаунчера без перезапуска на Android (activity-alias) и iOS (CFBundleAlternateIcons).
...
* Команда /pic для просмотра актуальной сезонной темы и получения аватара для бота.
...
* Динамический favicon в зависимости от сезона и праздников.
...
* Основные эндпоинты API:
  ...
  * `POST /api/app/auth/session` - генерация токена сессии авторизации через Telegram-бота.
  * `GET /api/app/auth/check` - проверка и подтверждение статуса входа в мобильном приложении.
  * `GET /api/app/profile`, `POST /api/app/profile` - получение и сохранение профиля пользователя.
  * `POST /api/app/device/sync` - регистрация и синхронизация параметров мобильного приложения.
  * `POST /api/app/device/unlink` - отвязка устройства от аккаунта Telegram при выходе.
  * `GET /api/app/version` - проверка актуальной версии мобильного приложения и получение ссылки на загрузку обновления.
  * `GET /api/app/changelog` - история изменений и релизов с GitHub Releases.
  * `GET /api/season/theme` - активная сезонная тема оформления.
...
│   │       ├── MainActivity.kt
│   │       └── ScheduleWidgetProvider.kt # Нативный виджет рабочего стола Android
│   ├── ios/                    # iOS-модуль проекта
│   │   ├── Runner/             # Основной проект Flutter iOS
│   │   └── ScheduleWidget/     # Нативный виджет рабочего стола WidgetKit
│   ├── assets/
│   │   └── icons/              # Сезонные и праздничные иконки (12 стилей)
...
│   │   ├── services/           # Сервисы API, кэширования, сезонных тем и виджета
...
│   ├── schedule.py             # Расписание на сегодня, завтра, недели, звонки, /pic
...
│   ├── season_service.py       # Определение сезонных тем и праздничных стилей
...
└── webapp/                     # Фронтенд Telegram Mini App
    ...
    └── app.js                  # Клиентская логика Mini App и сезонные favicon
```
